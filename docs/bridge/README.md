...
When the LLM router is enabled (see below), `GCP_API_KEY` must also be set on the bridge host - the router calls Gemini Pro (currently `gemini-3.1-pro-preview`) via BAML to classify each turn into a route. The shared backend credentials (`NEO4J_*`, `MYSQL_*`, `SESSION_DB_*`) are **no longer forwarded to the agent container**: they live **server-side on NExtSEEK**, which now runs the `chat_nextseek` agents on the agent's behalf (the A-5 rewire, 2026-06-14). The NExtSEEK sidecar that fronts the in-container plugin's granular operations is a thin HTTP forwarder holding only `NEXTSEEK_BASE_URL` and a staging directory — no backend credentials and no `chat_nextseek` runtime. In-container plugins reach NExtSEEK through the sidecar / assistant viewset, never through process-environment credentials. `GCP_API_KEY` stays on the bridge host (the router's BAML route-decision call); NExtSEEK holds its own copy server-side for the `chat_nextseek` agents.
...
- When `route="nextseek_query"`: no chat_nextseek process config remains in the container — the exec runs the thin runner (`runner_ns.py`), which calls the NExtSEEK assistant viewset over HTTP. The `chat_nextseek` pipeline runs server-side on NExtSEEK; the runner translates the viewset's events/terminal into the JSONL contract the bridge `ns_adapter` expects.
...
The in-container NS-route runner (`container/runner_ns.py`) performs an fd-shuffle at startup so its `print()` / debug output lands on docker exec **stderr**, while the JSONL events go to stdout. (Since the A-5 rewire the runner is a thin HTTP client to the NExtSEEK assistant viewset, not an in-process `chat_nextseek` host, so the structured-output parser debug it used to surface now runs server-side on NExtSEEK; the fd-shuffle still applies to any runner-side diagnostic output.) The bridge's `BridgeAttachSocket.read_event_line` reads both streams; stderr frames are not relayed to the chat UI (they may contain sensitive runtime debug data), and prior to 2026-05-18 they were truncated to 80 bytes and logged at DEBUG, which uvicorn's `--log-level error` discards entirely.
