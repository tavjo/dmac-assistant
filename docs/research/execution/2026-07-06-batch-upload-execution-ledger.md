...
| 4 | T3 runner/hard gate/resolution | remediation implemented after partial review; rereview pending | `a49ce68` + pending remediation commit | `out/T3.log`, `out/T3.xml`: T3 runner tests 27 passed, `_batch_upload_runner` coverage 99%, pin check pass; edited shell wrappers pass `sh -n`; deleted-flow token scan over `build_context/plugins/nextseek/bin` is clean | first review partial: semantic drift in pinned gate tests, missing-manifest and missing-processed gates too weak, unstructured fail-closed exceptions, non-atomic promotion, stale sample-read/merge shim surface, missing T3-local zero/both fixtures |
