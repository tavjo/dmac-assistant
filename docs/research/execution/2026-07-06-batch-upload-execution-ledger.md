...
| 4 | T3 runner/hard gate/resolution | second remediation implemented after partial rereview; rereview pending | `a49ce68`, `830e857` + pending second remediation commit | `out/T3.log`, `out/T3.xml`: T3 runner tests 27 passed, `_batch_upload_runner` coverage 99%, pin check pass; edited shell wrappers pass `sh -n`; deleted-flow token scan over `build_context/plugins/nextseek/bin` is clean | rereview partial: metadata-only updates with no manifest could still pass reusable artifact gate; ledger still needed remediation commit |
