...
| 2 | T1 client; T7 cost relay | remediation implemented after partial review; rereview pending | `ec6e840` + pending remediation commit | `out/T1.log`, `out/T1.xml`: T1 client tests 21 passed, `_batch_upload_client` coverage 99%, pin check pass; `out/T7.log`, `out/T7.xml`: explicit non-quarantined WebSocket regression set plus cost relay tests 91 passed, cost-relay and dispatch pin checks pass | first review partial: project tie-break could pick out-of-project assay, credential fallback mixed NEXTSEEK/API pairs, comma-title parser was naive, and T7 evidence used only 6 tests instead of the required explicit WebSocket file set |
