...
| 4 | T3 runner/hard gate/resolution | third remediation implemented after partial rereview; rereview pending | `a49ce68`, `830e857`, `d9e754e`, `ea32aff` | `out/T3.log`, `out/T3.xml`: T3 runner tests 28 passed, `_batch_upload_runner` coverage 99%, pin check pass; `test_check_pins.py` pass; edited shell wrappers pass `sh -n`; deleted-flow token scan over `build_context/plugins/nextseek/bin` and runner tests is clean | latest rereview partial: semantic pin coverage was still bundled, helper failures could be unstructured, create required fields were not independently enforced from workbook metadata; `ea32aff` remediates with named gate assertions, schema-aware create artifact gate, and wrapped helper command markers |
