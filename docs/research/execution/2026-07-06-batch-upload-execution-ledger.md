...
| 3 | T2 payload; T5 extract | remediation implemented after pass-with-debt review; rereview pending | `635aef3` + pending remediation commit | `out/T2.log`, `out/T2.xml`: T2 payload tests 17 passed, `_batch_upload_payload` coverage 98%, pin check pass; `out/T5.log`, `out/T5.xml`: T5 extract tests 13 passed, `_batch_upload_extract` coverage 100%, pin check pass | first review pass with spirit debt: review columns were not per-attribute and blank-UID rows could silently drop `attributes.UID` |
