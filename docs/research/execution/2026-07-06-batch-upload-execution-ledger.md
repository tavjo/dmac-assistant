...
| 6 | T8 $0 E2E; T9 image smoke | T8 implemented; T9 smoke harness implemented; image validation pending | `1af4cf7` + pending T9 smoke harness commit | `out/T8.log`, `out/T8.xml`: T8 tests 5 passed, `tools.e2e.run_batch_upload_e2e` coverage 96%, pin check pass | T9 image bake/smoke pending; use clean committed worktree for HEAD-anchored image parity because the main worktree has unrelated dirty plugin context files |
