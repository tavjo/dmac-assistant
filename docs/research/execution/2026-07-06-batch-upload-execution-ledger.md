...
| 5 | T4 shims; T6 skill contract | remediation implemented after partial review; rereview pending | `9f5d97a`, `10dc8ec` + pending remediation commit | `out/T4.log`, `out/T4.xml`: T4 shim tests 3 passed, pin check pass; `bash -n` and `shellcheck -x` pass on batch-upload shims; `out/T6.log`, `out/T6.xml`: T6 skill contract tests 3 passed | initial review partial: T4 tests used a manual runner table and weak dynamic forbidden-surface checks; T6 contradiction scanner was too fixed-phrase; remediation binds T4 to real `_CMDS`, adds static/dynamic forbidden-surface mutants, and broadens T6 synonym/distant contradiction fixtures |
