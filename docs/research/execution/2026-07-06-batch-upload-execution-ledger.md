...
| 4 | T3 runner/hard gate/resolution | fourth remediation implemented after partial rereview; rereview pending | `a49ce68`, `830e857`, `d9e754e`, `ea32aff`, `821dfa9`, `5a849be` | `out/T3.log`, `out/T3.xml`: T3 runner tests 27 passed, `_batch_upload_runner` coverage 98%, pin check pass; `out/T3-pin-selftest.log`: `test_check_pins.py` 9 passed; edited shell wrappers pass `sh -n`; deleted-flow token scan over `build_context/plugins/nextseek/bin` and runner tests is clean | latest rereview partial: new create-required source-of-truth regression was outside the required pin set, helper markers were generic, lettered gate semantics and ledger evidence were still loose; `5a849be` folds the regression into required pinned gate `k`, emits `assay_resolution`, adds project/token and project-tie-break semantics inside pinned gates, and records the ledger-only commit |
