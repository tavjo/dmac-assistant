...
| 5 | T4 shims; T6 skill contract | implemented; review pending | `9f5d97a` | `out/T4.log`, `out/T4.xml`: T4 shim tests 3 passed, pin check pass; `bash -n` and `shellcheck -x` pass on batch-upload shims; `out/T6.log`, `out/T6.xml`: T6 skill contract tests 3 passed | pending independent Wave 5 alignment review |
