...
| 7 | T9.5 live-fidelity probe | HELD after owner-approved live run; verifier failed insufficient breadth | `bd345c2`, `b79a243`, `2835392`, `df6020e` | `out/T9_5-verifier.log`, `out/T9_5-verifier.xml`: hermetic verifier self-tests 34 passed, `tools.e2e.verify_live_fidelity` coverage 96%; owner-approved live probe used `python-dotenv` with `.env.dev` and wrote `evidence/batch-upload-e2e/20260707T155647Z/live_fidelity_probe.json`; independent verifier command in `out/T9_5-live-verify.log` failed with `breadth probe returned fewer than three rows` because the W0 `breadth_probe_uids` bulk `advanced_search` returned `total=0`, `rows=[]` | HOLD/escalate per T9.5: breadth probe requires >=3 live rows; no merge on committed fixture alone; likely W0 breadth anchor drift or recapture needed before rerun |
